...
output_dir = os.path.join(output_dir, 'Continuum_1Exit_3Obs_DQN__Fully_Pytorch')
model_saved_path = os.path.join(model_saved_path, 'Continuum_1Exit_3Obs_DQN_Fully_Pytorch')
...
Ob1 = []
Ob1.append(np.array([0.5, 0.8, 0.5]))  # positioned near the exit
Ob.append(Ob1)
Ob_size.append(3.0)
...
# For linear decay: decay over 50% of training episodes
epsilon_decay_episodes = train_episodes * 0.5
...
save_step = 3000            # episodes to save model
...
if __name__ == '__main__':
    ...
    # Learning rate scheduler: decay LR by 0.95 every 1000 episodes.
    ...
    for ep in range(start_episode, train_episodes + 1):
        ...
        # # Corner sampling with probability 0.15
        # r = np.random.rand()
        # if r < 0.15:
        #     # Force the agent into one of the 4 "corner" subregions in normalized [0..1] coordinates:
        ...
        #     # Convert these normalized [0..1] coords to environment domain [0..GRID_SIZE]
        ...
        # Linear epsilon decay: decay over epsilon_decay_episodes episodes
        epsilon = max(explore_stop, explore_start - (explore_start - explore_stop) * (ep / epsilon_decay_episodes))
        ...
        while t < max_steps:
            ...
            # Training step: train once we have at least batch_size samples
        ...
        # End of episode: log average loss for the episode
        avg_loss = episode_loss_sum / episode_loss_count if episode_loss_count > 0 else 0.0
        episode_losses.append(avg_loss)
        episode_rewards.append(total_reward)

        if len(memory.buffer) >= batch_size:
            print(f"Episode: {ep}, Steps: {t}, Epsilon: {epsilon:.3f}, Reward: {total_reward:.2f}, Loss: {avg_loss:.7f}")
